...
st.set_page_config(page_title="Momentum Pro - Univers 150 (Anti-Biais)", layout="wide")
...
def run_momentum_pure():
    st.title("🚀 Momentum Pro : Univers 150 Valeurs (Réalisme Historique)")
    st.markdown("""
    **Univers de 150 titres :** Cette simulation inclut les leaders actuels, mais aussi les poids lourds des années 1960 à 2000 
    (dont certains ont fortement décliné). Cela élimine le biais de survie en forçant le script à choisir parmi les actifs disponibles à l'époque.
    """)
    
    # Univers de ~150 tickers mélant différentes époques (Tech, Industrie, Finance, Santé, Energie)
    universe_150 = [
        # --- TECH & TELECOMS (Anciens et Nouveaux) ---
        "AAPL", "MSFT", "GOOGL", "AMZN", "META", "NVDA", "INTC", "CSCO", "ORCL", "IBM", "HPQ", "TXN", "AMD", "MU", "NFLX", "TSLA", "ADBE", "CRM", "PLTR", "AVGO",
        "SAP", "ASML", "QCOM", "AMAT", "LRCX", "ADI", "PANW", "SNPS", "CDNS", "VRSN", "T", "VZ", "TMUS", "ERIC", "NOK", "CSGP", "NOW", "UBER", "SHOP", "SQ",
        # --- FINANCE & PAIEMENT ---
        "JPM", "BAC", "GS", "MS", "AXP", "V", "MA", "WFC", "C", "BRK-B", "PYPL", "BLK", "SPGI", "CB", "MMC", "PGR", "MET", "PRU", "AIG", "SCHW",
        # --- INDUSTRIE & TRANSPORT ---
        "GE", "XOM", "CVX", "CAT", "BA", "MMM", "HON", "LMT", "DE", "F", "GM", "UPS", "FDX", "RTX", "GD", "NOC", "WM", "RSG", "NSC", "UNP", "CSX", "ETN", "PH", "EMR", "ITW",
        # --- CONSOMMATION & RETAIL ---
        "WMT", "KO", "PEP", "PG", "COST", "TGT", "HD", "MCD", "NKE", "DIS", "SBUX", "LOW", "TJX", "EL", "CL", "KMB", "MO", "PM", "STZ", "MDLZ", "MAR", "HLT", "BKNG", "YUM",
        # --- SANTÉ & PHARMA ---
        "JNJ", "PFE", "LLY", "UNH", "ABBV", "MRK", "AMGN", "CVS", "ISRG", "GILD", "VRTX", "REGN", "ZTS", "SYK", "BSX", "BDX", "HCA", "TMO", "DHR", "A",
        # --- ÉNERGIE & MATÉRIAUX ---
        "COP", "SLB", "EOG", "MPC", "PSX", "VLO", "NEM", "FCX", "APD", "LIN", "SHW", "CTAS", "DD", "DOW", "ALB",
        # --- HISTORIQUES / DÉCHUS / CYCLIQUES ---
        "X", "F", "GM", "WRE", "BSX", "AA", "HAL", "BKR", "K", "GIS", "CPB", "ADM", "IP", "WY", "M", "KSS", "JWN"
    ]
    universe_150 = list(set(universe_150))

    with st.sidebar:
        st.header("⚙️ Paramètres Stratégie")
        n_top = st.slider("Nombre d'actions à détenir", 1, 30, 10)
    ...
    try:
        with st.spinner(f'Téléchargement et analyse de {len(universe_150)} titres...'):
            close_data, open_data, spy_sma = load_data(start_date, end_date, lookback, sma_period)
            if close_data.empty: return

            monthly_close = close_data.resample('ME').last()
            # On ne calcule le momentum que sur les colonnes de l'univers
        ...
        st.subheader("🏁 Performance Comparative (Univers 150)")
        st.table(pd.DataFrame([m_s, m_b], index=["Stratégie Momentum Multi-Décennale", "S&P 500 (^GSPC)"]).T)

        st.subheader("📈 Évolution Portefeuille (Échelle Log)")
        cum_data = (1 + df[['Ma Stratégie', 'S&P 500']]).cumprod() * 100
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=cum_data.index, y=cum_data['Ma Stratégie'], name="Ma Stratégie", line=dict(color='#00b4d8', width=2.5)))
        fig.add_trace(go.Scatter(x=cum_data.index, y=cum_data['S&P 500'], name="S&P 500", line=dict(color='#ffb703', width=1.5, dash='dot')))
        fig.update_layout(yaxis_type="log", template="plotly_white", height=600, hovermode="x unified")
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("🔍 Historique des Choix (Univers Dynamique)")
